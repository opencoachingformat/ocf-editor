...
/**
 * Validate all examples/*.ocf.json files against the OCF schema.
 * Replicates the structural validation logic from src/export/validate.js
 * as a standalone CommonJS script (no extra dependencies required).
 * Exit code 1 if any validation errors are found (CI-compatible).
 */
...
if (!fs.existsSync(examplesDir)) {
  console.log('No examples/ directory found. Skipping validation.');
  process.exit(0);
}

const files = fs.readdirSync(examplesDir).filter(f => f.endsWith('.ocf.json'));

if (files.length === 0) {
  console.log('No *.ocf.json files found in examples/. Skipping validation.');
  process.exit(0);
}
...
for (const file of files) {
  ...
  try {
    ...
    console.error(`FAIL  ${file}: JSON parse error — ${err.message}`);
    allValid = false;
    continue;
  }

  const { valid, errors } = validateOCF(doc);
  if (valid) {
